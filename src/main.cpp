...
void DidActivate(HMUI::ViewController* self, bool firstActivation, bool addedToHierarchy, bool screenSystemEnabling){
    if(firstActivation){
        ...
        BeatSaberUI::CreateToggle(container->get_transform(), "Active", getMainConfig().Active.GetValue(),
            [](bool value) { 
                getMainConfig().Active.SetValue(value);
            });

        BeatSaberUI::CreateToggle(container->get_transform(), "One Handed Mode", getMainConfig().OneHand.GetValue(),
            [](bool value) { 
                getMainConfig().OneHand.SetValue(value);
            });

        BeatSaberUI::CreateToggle(container->get_transform(), "Left Handed Mode", getMainConfig().LeftHanded.GetValue(),
            [](bool value) { 
                getMainConfig().LeftHanded.SetValue(value);
            });
    }
}
